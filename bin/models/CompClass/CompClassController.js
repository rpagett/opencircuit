...
router.get('/table', function (req, res) {
  var contents = {};
  _CompClassModel2.default.find({}, '_id name abbreviation unit_type detailsUrl').populate('unit_type', 'name').sort('unit_type name').exec().then(function (classes) {
    var ids = [];
...
router.route('/:abbreviation').get(function (req, res) {
  ...
  _CompClassModel2.default.findOneAndRemove({ abbreviation: req.params.abbreviation }).exec().then(function () {
    res.json({
      success: true
    });
  }).catch(function (err) {
